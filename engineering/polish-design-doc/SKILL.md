...
- 採用案が概要・詳細設計へ昇格し、既知の不採用案がある場合は、その案と理由が `検討した案` に残っている。
- `TODO(polish)` が残っていない。
- task 分割に進める。
...
既に採用案が決まっていれば、その判断と根拠を使う。未決定の場合は、判断事項、候補、比較軸、根拠の不足を整理する。
候補や比較軸に不足があれば調査で補う。
...
採用案が未決定の場合だけ、`検討した案` にある候補を比較し、次をユーザーへ提示する。
...
既にユーザー指定・会話・参照文書で採用案が確定している場合は、この再確認を省き、根拠を反映して更新する。新たな要求・制約との矛盾が見つかった場合だけ、その判断を確認する。
未決定の場合は、全文previewの再承認ではなく採用案の判断を求め、それに依存する更新だけを保留する。
