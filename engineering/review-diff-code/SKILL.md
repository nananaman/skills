...
2. helperを実行する。Context Builderはraw diffとrepositoryをread-onlyで調べ、changed fileをimplementation、issue context、未分類へ分け、関連するrepository内issueと実装を抽出する。
   - implementation fileごとにcaller、consumer、test、contractの調査結果を記録する。
   - 未分類file、分類の重複や漏れ、未解決の影響、不正なpath / excerpt / schemaがあればreviewerを起動せず停止する。
...
- `Behavioral Safety`: implementation diff、issue context、関連実装を受け取り、correctness、regression、security、type / API contract、verification gapを扱う。
- `Design Quality`: implementation diff、issue context、関連実装を受け取り、ownership boundary、maintainability、structure、behavior-preserving simplificationを扱う。
...
- Context Builderだけがrepositoryをworking directoryにし、read toolでraw diff、issue情報、caller、consumer、test、contractを調べる。
- 3 reviewerは空のworking directoryとno-tools optionを使い、repositoryを追加調査しない。
- Behavioral SafetyとDesign Qualityにはimplementation diff、issue context、関連実装を渡す。
