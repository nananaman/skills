...
description: 現在の diff、branch diff、commit diff、PR diff を対象に、correctness・security・maintainability・構造品質を厳しめにレビューする。コードレビュー、PR レビュー、別モデルレビュー、保守性レビュー、実装後の closeout review を依頼されたときに使う。
---
...
```bash
~/.agents/skills/review-diff-code/scripts/review-diff-code --engine codex --mode branch --base origin/main
~/.agents/skills/review-diff-code/scripts/review-diff-code --engine claude --mode branch --base origin/main
