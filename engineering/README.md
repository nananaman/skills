...
- コーディング規範を ast-grep の project-local な構造 lint にし、rule / rewrite を運用する → [`ast-grep-practice`](./ast-grep-practice/SKILL.md)
...
- **[`nono-sandbox-maintenance`](./nono-sandbox-maintenance/SKILL.md)** — nono の拒否を診断し、最小権限の profile patch を作成・検証する。
  ...
  - Type: `model-invoked`
- **[`ast-grep-practice`](./ast-grep-practice/SKILL.md)** — コーディング規範を ast-grep の project-local な構造 lint にし、rule / rewrite を運用する。
