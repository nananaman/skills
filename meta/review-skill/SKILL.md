---
name: review-skill
description: skill 全体を棚卸しして、責務過多、archetype fit、metadata discoverability、positive / negative trigger、description / 本文不一致、progressive disclosure、deterministic resources、reference / diagnostic quality、誤発火・不発火、safety gap を洗い出す。skill の大幅変更、新規 skill、または「この skill が悪そう」という調査で使う。
---
...
1. 対象 skill と目的を確認する。
   ...
   - skill archetype（workflow / reference / diagnostic / rubric / wrapper）を判定し、本文構造と合っているか確認する。
...
5. 全体構造を評価する。
   ...
   - workflow / rubric / wrapper skill では、`SKILL.md` が通常 500 行未満で、agent の初期 context に置くべき手順へ絞られているか。
   - reference / diagnostic skill では、対象 tool / product / API、利用者、対象外領域、条件差分、頻出 recipe、制限、troubleshooting、quick reference が本文でたどれるか。
   - reference / diagnostic skill の command 例が copy-paste 可能で、destructive command や永続変更に確認条件・注意があるか。
   - 外部資料由来の reference skill では、upstream URL、取得日、license / attribution、更新時の確認方法が必要に応じて残っているか。
...
- Archetype: 判定した archetype と構造評価
