...
description: skill 変更 diff を commit / push / APM pin / install 前に必ずレビューする。metadata discoverability、positive / negative trigger、progressive disclosure、deterministic resources、safety、配布導線の regression を検出し、actionable finding が残る間は先へ進ませない。
---
...
5. さらに高リスクなら `reviewing-skills` の Level 2: Independent executor trial を実行する。
   ...
   - 高重要度 skill の大幅変更では、3 回連続 clear、hold-out scenario、tool-use 偏り確認まで行う。
...
```md
## Evaluation
- reviewing-skills Level: Level 0 / Level 1 / Level 2 のどこまで実行したか。高重要度なら収束判定と hold-out scenario の結果
