...
description: skill 変更 diff をレビューする。対象 diff が skill の archetype、routing、判断、手順、停止条件、出力形式、reference / diagnostic quality に影響するかを冒頭で判定し、影響する場合は reviewing-skills の static contract check と smoke check で regression を検出する。単なる文書整理や実行契約に影響しない変更は skip decision で止める。
---
...
2. Applicability check を行う。
   ...
   - diff が次のいずれかを変えるか判定する。
     ...
     - reference / diagnostic quality（command map、recipe、制限、条件差分、troubleshooting、unsafe command など）。
...
3. Static contract check を実行する。
   ...
   - skill archetype（workflow / reference / diagnostic / rubric / wrapper）と本文構造が、変更後も一致しているか確認する。
...
6. diff 固有の regression を見る。
   ...
   - reference / diagnostic skill で、quick reference と詳細説明の矛盾、バージョン・OS・権限などの条件削除、unsafe な copy-paste command、troubleshooting の行き止まりが増えていないか。
   - 外部資料由来の reference skill で、upstream URL、取得日、license / attribution、更新確認方法が必要な変更なのに欠けていないか。
