...
- 運用中の挙動やグローバル指示との整合を扱う監査では、manifest の参照先と実際に読み込まれる展開先を確認する。正本だけのレビューでは展開先の確認を省略してよい。
...
1. 対象 repository と補助確認の有無を決める。
   ...
   - 確認する場合は、manifest が指定する正本と展開先の本文・関連 resources を比較する。差異を報告し、監査の依頼だけでは install や展開先の修正をしない。
