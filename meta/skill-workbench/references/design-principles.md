...
別 skill を参照するときは、実行環境の skill 一覧にある実際のパスを優先する。
一覧にない場合だけ、展開先の兄弟ディレクトリまたは正本 repository の category 階層から候補を確認する。相対パスが両方の配置で通用すると仮定しない。
明示起動の skill を内部手順で読む場合も、呼び出し元の依頼がその成果と操作を含むことを確認する。
...
承認は対象・操作・範囲・回数に対する許可として扱う。会話内の依頼・採用指示を再利用し、一回限り・都度承認の指定や、入力・影響が承認範囲から外れる変更では新たな許可を確認する。
停止条件は依存する操作を特定し、独立した作業の継続と未検証事項の報告を区別する。
