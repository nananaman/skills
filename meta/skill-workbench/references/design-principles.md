...
- success / completion criteria: 終了前に満たす状態。
...
各項目を見出しとして機械的に追加しない。
agent の判断を変える項目だけを通常 path または該当 branch に置き、暗黙でも一意に判断できる項目は重ねて説明しない。
...
| Contradiction | 同じ状況に複数の相反する rule が適用される | preservation set と prompt contract を基準に優先順位または decision rule を固定する |
