...
| 介入候補の探索 | 外部資料中の技法候補と観測症状の対応、採否、候補漏れ、資料の適用範囲 |
...
- 振り分け、分岐ごとの完了条件、権限と安全が、維持対象またはユーザーが許可した契約変更と一致することを確認する。
...
- 外部資料を主要な根拠にした場合は、対象の主要症状について、資料中の関連技法が採用、不採用、要評価のいずれかとして検討されているか確認する。
- 最初の原因仮説だけを補強し、より直接的な技法、代替、組み合わせ、退行リスクを見落としていないか確認する。
...
実施前に入力、期待する外部動作、禁止する副作用を固定する。共通契約の変更は、変更の影響を代表する分岐と境界条件を選び、同じ意味の例を skill ごとに増やさない。
必要な検査が成功した後は、新しい変更・失敗・未解決の懸念がある場合だけ検査を拡大または再実行する。
