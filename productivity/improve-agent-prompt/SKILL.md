...
| 提案 | 改善案や変更案を求められた | 保存対象を維持した改訂案または差分案、根拠、検証方法を会話内で返した。ファイルは変更しない |
| 改善 | 書き換え、実際の編集を求められた | 保存対象を維持した変更を適用し、検証結果を返した |

依頼が診断、計画、レビューだけなら prompt を変更しない。
model や provider の指定だけで実装範囲を広げない。観測された失敗または確認できた契約の欠落を根拠にする。
公式の model 固有ガイドが指定された場合は、その資料に明記された技法を評価してよい。適用条件、対象での根拠、期待する変化、退行リスクを対応づけ、未検証の効果を実証済みと扱わない。根拠のない provider 固有の調整は導入しない。
