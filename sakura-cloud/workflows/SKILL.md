...
さくらのクラウド Workflows の運用手順作成、デバッグ、API 操作のための作業手順。
...
- 対象ゾーンはどこか。例では `tk1b` を使う。

## YAML 作成後の確認

- `assign` の同一ブロック参照、式構文、HTTP レスポンス本文のデコード要否を `REFERENCE.md` の落とし穴に照らして確認する。
- 公式の検証 API や試行実行が使えるか不明な場合は、いきなり本番実行せず、実行前レビュー用の YAML と確認観点をユーザーに提示する。

## 外部操作の権限

- 作成・実行・キャンセルには、対象ゾーン・workflow/execution ID・引数・影響範囲への明示的な許可が必要。同じ範囲の会話内の許可は引き継ぐ。未許可なら具体的な案を示し、API操作を保留する。YAMLの編集依頼だけを本番実行の許可にしない。
...
公式 OpenAPI JSON はこの skill の repository に同梱しない。必要なときに公式 URL から一時取得する。
